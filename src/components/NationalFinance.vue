...
<template>
    <div class="container">
      <div class="buttons">
        <button class="button" :class="{ 'is-active': isBarActive_Finance }" @click="drawBarChart_Finance" style="margin-top: 50px;" >柱状图</button>
        <button class="button" :class="{ 'is-active': isLineActive_Finance }" @click="drawLineChart_Finance" style="margin-top: 50px;">折线图</button>
      </div>
      <div class="chart-container" id="finance"></div>
  
       <!-- 为下方的按钮添加上边距 style="margin-top -->
      <div class="buttons">
        <button class="button" :class="{ 'is-active': isBarActive_FiscalRevenue }" @click="drawBarChart_FiscalRevenue" style="margin-top: 50px;">柱状图</button>
        <button class="button" :class="{ 'is-active': isLineActive_FiscalRevenue }" @click="drawLineChart_FiscalRevenue" style="margin-top: 50px;">折线图</button>
      </div>
      <div class="chart-container" id="fiscalrevenue"></div>
  
       <!-- 为下方的按钮添加上边距 style="margin-top -->
      <div class="buttons">
        <button class="button" :class="{ 'is-active': isBarActive_FiscalExpenditure }" @click="drawBarChart_FiscalExpenditure" style="margin-top: 50px;">柱状图</button>
        <button class="button" :class="{ 'is-active': isLineActive_FiscalExpenditure }" @click="drawLineChart_FiscalExpenditure" style="margin-top: 50px;">折线图</button>
      </div>
      ...
      export default {
        ...
        methods: {
          loadData() {
            // 请求人民收入公开数据
            fetch('nationalFinanceData.json')
              .then(response => response.json())
              .then(data => {
                console.log('请求成功：财政收入与支出数据:',data.nationalFinanceData);
                // 列表数据
                this.nationalFinanceDataList = data.nationalFinanceData;
                // 处理数据绘制图表
          ...
          //按照年份与日期做筛选与排序
          nationalFinanceArr(type, showX = 0) {
            return this.nationalFinanceDataList.filter( nationalFinanceDataListObj => {
              return nationalFinanceDataListObj.code.search(type) != -1 && nationalFinanceDataListObj.value !=0;
          }).sort(function(a,b) {
          ...
          drawFinanceCharts() {
            ...
            var financeOption = {
                ...
                xAxis: {
                    type: 'category',
                    data: this.nationalFinanceArr(this.NationalFinance.A080201, 1)
                },
                yAxis: {
                },
                series: [
          ...
          drawFiscalRevenueCharts() {
            ...
            var fiscalRevenueOption = {
                ...
                xAxis: {
                    type: 'category',
                    data: this.nationalFinanceArr(this.NationalFinance.A08040102, 1)
                },
                yAxis: {
