...
export const resolveMapType = (localJsonPath) => {
  ...
  if (localJsonPath.includes('wh.json')) return 'wuhan';
  if (localJsonPath.includes('province.json')) return 'province';
  if (localJsonPath.includes('city.json')) return 'city';
  return null;
};
...
export const registerAllMaps = async () => {
  try {
    ...
    // 使用 Promise.all 并发发起 3 个请求，极致压缩加载时间
    // 注意：这里的路径以 / 开头，代表直接访问 public 目录下的文件
    const [whRes, provinceRes, cityRes] = await Promise.all([
      axios.get('/json/geo_wh.json'),
      axios.get('/json/geo_province.json'),
      axios.get('/json/geo_city.json')
    ]);

    // 拿到数据后，注册到 ECharts
    echarts.registerMap('wuhan', whRes.data);
    echarts.registerMap('province', provinceRes.data);
    echarts.registerMap('city', cityRes.data);
